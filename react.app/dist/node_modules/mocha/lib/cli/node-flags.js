...
var nodeFlags = process.allowedNodeEnvironmentFlags;

var _require = require('./run-option-metadata'),
    isMochaFlag = _require.isMochaFlag;
 ...
 * @private
 */
var debugFlags = new Set(['inspect', 'inspect-brk']);
 ...
 * @private
 */
exports.isNodeFlag = function (flag) {
  var bareword = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  return (
    ...
    !isMochaFlag(flag) && nodeFlags && nodeFlags.has(flag) || debugFlags.has(flag) || /(?:preserve-symlinks(?:-main)?|harmony(?:[_-]|$)|(?:trace[_-].+$)|gc[_-]global$|es[_-]staging$|use[_-]strict$|v8[_-](?!options).+?$)/.test(flag)
  );
};
 ...
 * @private
 */
exports.impliesNoTimeouts = function (flag) {
 ...
 * @private
 */
exports.unparseNodeFlags = function (opts) {
  var args = unparse(opts);
  return args.length ? args.join(' ').split(/\b/).map(function (arg) {
    return arg === ' ' ? '=' : arg;
  }).join('').split(' ') : [];
};