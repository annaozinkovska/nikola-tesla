...
var _require = require('./errors'),
    ...
    errorConstants = _require.constants;
...
var Runner = function (_EventEmitter) {
  ...
  function Runner(suite) {
    ...
    _this.suite = suite;
    _this._opts = opts;
    _this.state = constants.STATE_IDLE;
    ...
    _this.on(constants.EVENT_TEST_END, function (test) {
    ...
    _this.on(constants.EVENT_HOOK_END, function (hook) {
    ...
    _this.uncaught = _this._uncaught.bind(_this);
    _this.unhandled = function (reason, promise) {
      if (isMochaError(reason)) {
        ...
        _this._removeEventListener(process, 'unhandledRejection', _this.unhandled);
        try {
          process.emit('unhandledRejection', reason, promise);
        } finally {
          _this._addEventListener(process, 'unhandledRejection', _this.unhandled);
        }
      }
    };
    return _this;
...
Runner.prototype.checkGlobals = function (test) {
  ...
  if (leaks.length) {
    var msg = 'global leak(s) detected: ' + leaks.map(function (e) {
      return '\'' + e + '\'';
    }).join(', ');
...
Runner.prototype.run = function (fn) {
  ...
  var end = function end() {
    ...
    _this2.emit(constants.EVENT_RUN_END);
  };

  var begin = function begin() {
    debug('run(): emitting %s', constants.EVENT_RUN_BEGIN);
    _this2.emit(constants.EVENT_RUN_BEGIN);
  ...
  var prepare = function prepare() {
    ...
    _this2.state = constants.STATE_RUNNING;
    if (_this2._opts.delay) {
      _this2.emit(constants.EVENT_DELAY_END);
 ...
 */
Runner.prototype.runAsync = function () {
