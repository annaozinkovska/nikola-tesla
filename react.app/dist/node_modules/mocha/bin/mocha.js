...
var _require = require('../lib/cli/options'),
    loadOptions = _require.loadOptions;

var _require2 = require('../lib/cli/node-flags'),
    unparseNodeFlags = _require2.unparseNodeFlags,
    isNodeFlag = _require2.isNodeFlag,
    impliesNoTimeouts = _require2.impliesNoTimeouts;

var unparse = require('yargs-unparser');
var debug = require('debug')('mocha:cli:mocha');

var _require3 = require('../lib/cli/run-option-metadata'),
    aliases = _require3.aliases;

var mochaArgs = {};
var nodeArgs = {};
var hasInspect = false;

var opts = loadOptions(process.argv.slice(2));
 ...
 * @ignore
 */
var disableTimeouts = function disableTimeouts(value) {
 ...
 * @ignore
 */
var trimV8Option = function trimV8Option(value) {
  return value !== 'v8-options' && /^v8-/.test(value) ? value.slice(3) : value;
};
...
Object.keys(nodeArgs).forEach(function (opt) {
  return disableTimeouts(opt);
});
mochaArgs['node-option'] && mochaArgs['node-option'].forEach(function (opt) {
  return disableTimeouts(opt);
});
...
if (mochaArgs._) {
  var i = mochaArgs._.findIndex(function (val) {
    return val === 'inspect';
  });
  if (i > -1) {
...
if (mochaArgs['node-option'] || Object.keys(nodeArgs).length || hasInspect) {
  ...
  var mochaPath = require.resolve('../lib/cli/cli.js');

  var nodeArgv = mochaArgs['node-option'] && mochaArgs['node-option'].map(function (v) {
    return '--' + v;
  ...
  var args = [].concat(nodeArgv, mochaPath, unparse(mochaArgs, { alias: aliases }));

  debug('forking child process via command: %s %s', process.execPath, args.join(' '));
