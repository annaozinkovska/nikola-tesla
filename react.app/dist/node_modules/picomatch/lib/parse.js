...
var parse = function parse(input, options) {
  ...
  var extglobOpen = function extglobOpen(type, value) {
    var token = Object.assign({}, EXTGLOB_CHARS[value], { conditions: 1, inner: '' });
  ...
  var extglobClose = function extglobClose(token) {
    ...
    if (token.type === 'negate') {
      ...
      if (token.inner.includes('*') && (rest = remaining()) && /^\.[^\\/.]+$/.test(rest)) {
        ...
        var expression = parse(rest, Object.assign({}, options, { fastpaths: false })).output;
  ...
  while (!eos()) {
    ...
    if (value === '}') {
      ...
      push({ type: 'brace', value: value, output: _output });
      decrement('braces');
      braces.pop();
      continue;
    }
    ...
    if (value === ',') {
      ...
      push({ type: 'comma', value: value, output: _output2 });
      continue;
    }
    ...
    if (value === '?') {
      ...
      if (prev && prev.type === 'paren') {
        ...
        push({ type: 'text', value: value, output: _output3 });
        continue;
      }
