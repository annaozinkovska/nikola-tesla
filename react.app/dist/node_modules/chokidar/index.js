...
var unifyPaths = function unifyPaths(paths_) {
  ...
  if (!paths.every(function (p) {
    return (typeof p === 'undefined' ? 'undefined' : _typeof(p)) === STRING_TYPE;
  })) {
...
var normalizeIgnored = function normalizeIgnored() {
  ...
  return function (path) {
    if ((typeof path === 'undefined' ? 'undefined' : _typeof(path)) !== STRING_TYPE) return path;
...
var DirEntry = function () {
  ...
  _createClass(DirEntry, [{
    ...
    value: function getChildren() {
      ...
      if (!items) return;
      return [].concat(_toConsumableArray(items.values()));
    }
  }, {
    key: 'dispose',
...
var FSWatcher = function (_EventEmitter) {
  ...
  function FSWatcher(_opts) {
    ...
    var _this2 = _possibleConstructorReturn(this, (FSWatcher.__proto__ || Object.getPrototypeOf(FSWatcher)).call(this));
    ...
    _this2._emitRaw = function () {
      ...
      return _this2.emit.apply(_this2, [EV_RAW].concat(args));
    };
    ...
    if (opts.useFsEvents) {
      _this2._fsEventsHandler = new FsEventsHandler(_this2);
    } else {
      _this2._nodeFsHandler = new NodeFsHandler(_this2);
    }
  ...
  _createClass(FSWatcher, [{
    ...
    value: function add(paths_, _origAdd, _internal) {
      ...
      var _options = this.options,
          cwd = _options.cwd,
          disableGlobbing = _options.disableGlobbing;

      this.closed = false;
      var paths = unifyPaths(paths_);
      if (cwd) {
        paths = paths.map(function (path) {
          var absPath = getAbsolutePath(path, cwd);
      ...
      paths = paths.filter(function (path) {
        if (path.startsWith(BANG)) {
          _this3._ignoredPaths.add(path.slice(1));
          return false;
        }

        // if a path is being added that was previously ignored, stop ignoring it
        _this3._ignoredPaths.delete(path);
        _this3._ignoredPaths.delete(path + SLASH_GLOBSTAR);

        // reset the cached userIgnored anymatch fn
        // to make ignoredPaths changes effective
        _this3._userIgnored = undefined;
      ...
      if (this.options.useFsEvents && this._fsEventsHandler) {
        ...
        paths.forEach(function (path) {
          return _this3._fsEventsHandler._addToFsEvents(path);
        });
      } else {
        ...
        Promise.all(paths.map(function () {
            ...
            return _regeneratorRuntime.wrap(function _callee2$(_context2) {
              while (1) {
                switch (_context2.prev = _context2.next) {
                  case 0:
                    _context2.next = 2;
                    return _this3._nodeFsHandler._addToNodeFs(path, !_internal, 0, 0, _origAdd);

                  case 2:
                    res = _context2.sent;

                    if (res) _this3._emitReady();
          ...
          results.filter(function (item) {
            ...
            _this3.add(sysPath.dirname(item), sysPath.basename(_origAdd || item));
          });
        });
      }
    ...
    */
    ...
    value: function unwatch(paths_) {
      ...
      paths.forEach(function (path) {
        ...
        if (!sysPath.isAbsolute(path) && !_this4._closers.has(path)) {
          if (cwd) path = sysPath.join(cwd, path);
          path = sysPath.resolve(path);
        }

        _this4._closePath(path);

        _this4._ignoredPaths.add(path);
        if (_this4._watched.has(path)) {
          _this4._ignoredPaths.add(path + SLASH_GLOBSTAR);
        }
    ...
    */
    ...
    value: function getWatched() {
      ...
      this._watched.forEach(function (entry, dir) {
        var key = _this6.options.cwd ? sysPath.relative(_this6.options.cwd, dir) : dir;
    ...
    key: '_emit',
    value: function () {
      var _ref3 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime.mark(function _callee3(event, path, val1, val2, val3) {
        var _this7 = this;
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              ...
              case 12:
                ...
                setTimeout(function () {
                  _this7._pendingUnlinks.forEach(function (entry, path) {
                    _this7.emit.apply(_this7, _toConsumableArray(entry));
                    _this7.emit.apply(_this7, [EV_ALL].concat(_toConsumableArray(entry)));
                    _this7._pendingUnlinks.delete(path);
                  });
      ...
      function _emit(_x6, _x7, _x8, _x9, _x10) {
    ...
    value: function _throttle(actionType, path, timeout) {
      ...
      var thr = { timeoutObject: timeoutObject, clear: clear, count: 0 };
      action.set(path, thr);
      return thr;
    }
  }, {
    ...
    value: function _awaitWriteFinish(path, threshold, event, awfEmit) {
      ...
      var awaitWriteFinish = function awaitWriteFinish(prevStat) {
        fs.stat(fullPath, function (err, curStat) {
          if (err || !_this8._pendingWrites.has(path)) {
          ...
          if (prevStat && curStat.size !== prevStat.size) {
            _this8._pendingWrites.get(path).lastChange = now;
          }
          var pw = _this8._pendingWrites.get(path);
          var df = now - pw.lastChange;

          if (df >= threshold) {
            _this8._pendingWrites.delete(path);
            awfEmit(undefined, curStat);
          } else {
            timeoutHandler = setTimeout(awaitWriteFinish, _this8.options.awaitWriteFinish.pollInterval, curStat);
          }
        });
      };
    ...
    value: function _getGlobIgnored() {
      return [].concat(_toConsumableArray(this._ignoredPaths.values()));
    }
    ...
    value: function _isIgnored(path, stats) {
      ...
      if (!this._userIgnored) {
        ...
        var paths = arrify(ignored).filter(function (path) {
          return (typeof path === 'undefined' ? 'undefined' : _typeof(path)) === STRING_TYPE && !isGlob(path);
        }).map(function (path) {
    ...
    */
    ...
    value: function _remove(directory, item, isDirectory) {
      ...
      var nestedDirectoryChildren = wp.getChildren();

      // Recursively remove children directories / files.
      nestedDirectoryChildren.forEach(function (nested) {
        return _this9._remove(path, nested);
      });
    ...
    value: function _readdirp(root, opts) {
      ...
      var options = Object.assign({ type: EV_ALL, alwaysStat: true, lstat: true }, opts);
      ...
      stream.once(STR_END, function () {
        if (stream) {
          _this10._streams.delete(stream);
          stream = undefined;
        }
      });
      return stream;
