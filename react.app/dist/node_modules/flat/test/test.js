...
suite('Arrays', function () {
  ...
  test('Array typed objects should be restored by unflatten', function () {
    assert.strictEqual(Object.prototype.toString.call(['foo', 'bar']), Object.prototype.toString.call(unflatten({
      'a.0': 'foo',
      'a.1': 'bar'
    }).a));
  });
...
suite('Order of Keys', function () {
  test('Order of keys should not be changed after round trip flatten/unflatten', function () {
    ...
    var result = unflatten(flatten(obj));
...
suite('CLI', function () {
  test('can take filename', function (done) {
    var cli = path.resolve(__dirname, '..', pkg.bin);
    var pkgJSON = path.resolve(__dirname, '..', 'package.json');
    exec(cli + ' ' + pkgJSON, function (err, stdout, stderr) {
      assert.ifError(err);
  ...
  test('can take piped file', function (done) {
    var cli = path.resolve(__dirname, '..', pkg.bin);
    var pkgJSON = path.resolve(__dirname, '..', 'package.json');
    exec('cat ' + pkgJSON + ' | ' + cli, function (err, stdout, stderr) {
      assert.ifError(err);
      assert.strictEqual(stdout.trim(), JSON.stringify(flatten(pkg), null, 2));
      done();
    });
